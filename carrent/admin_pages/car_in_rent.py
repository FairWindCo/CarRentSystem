...
class ReturnCarRentPage(CustomModelPage):
    ...
    def clean(self):
        ...
        if return_many <= 0 or not self.car_rent.my_break_rent:
        ...
        if self.deposit_cash_box.get_current_balance() < self.car_rent.deposit:
            raise ValidationError(_('В выбранной кассе не достаточно средств'))
        super().clean()
    ...
    def save(self):
        with transaction.atomic():
            ...
            if Balance.form_transaction(Balance.WITHDRAWAL, [
                (self.cash_box, None, return_many, f'Возврат денег за не полную аренду  {self.car_rent.car.name}'),
                (self.deposit_cash_box, None, self.car_rent.deposit,
                 f'Возврат залога за аренду {self.car_rent.car.name}'),
                (self.car_rent.car, None, return_many, f'Возврат денег за не полную аренду  {self.car_rent.car.name}'),
            ], 'Аренда автомобиля'):
                self.bound_admin.message_success(self.bound_request, _('Аренда завершена'))


class ReturnCarRentPageAdmin(CustomPageModelAdmin):
    autocomplete_fields = ('car_rent',)
...
class AddDepositCarRentPage(CustomModelPage):
    ...
    def clean(self):
        ...
        if self.car_rent.paid_deposit + self.amount > self.car_rent.deposit:
            raise ValidationError(_('Превишен депозит'))

    def save(self):
        with transaction.atomic():
            self.car_rent.paid_deposit += self.amount
            self.car_rent.save()
            amount = round(self.amount * 100)
            if Balance.form_transaction(Balance.WITHDRAWAL, [
                (None, self.cash_box, amount, f'Доплата депозита {self.car_rent.car.name}'),
            ], 'Аренда автомобиля'):
...
class PenaltyDepositCarRentPage(CustomModelPage):
    ...
    def save(self):
        with transaction.atomic():
            ...
            amount = round(self.amount * 100)
            if Balance.form_transaction(Balance.WITHDRAWAL, [
                (None, self.car_rent.car, amount, f'Штраф с депозита {self.car_rent.car.name}'),
            ], 'Аренда автомобиля'):
                self.bound_admin.message_success(self.bound_request, _('Доплата депозита'))
